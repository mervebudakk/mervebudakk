...
- **Languages:** C#, Python, Dart, TypeScript, SQL (T-SQL, PL/pgSQL), JavaScript
- **Backend & APIs:** ASP.NET Core, FastAPI, NestJS, RESTful Architecture, JWT Security, Entity Framework
- **Frontend & Mobile:** Flutter, Next.js, React, WPF
- **Data & Algorithms:** PostgreSQL, MS SQL, MongoDB, Graph Algorithms (Dijkstra, Clark-Wright), Object-Oriented Design
- **AI & NLP:** PyTorch, OpenCV, HuggingFace Transformers, BERTTurk, GLiNER
- **Tools & DevOps:** Git, Docker, Docker Compose, Nginx, Redis, Supabase, Linux
...
| **Enterprise Building Management System** <br> <sub>[API](https://github.com/mervebudakk/Bina-Yonetim-Sistemi-API) • [Database](https://github.com/mervebudakk/Bina-Yonetim-Sistemi-Veritabani) • [Mobile](https://github.com/mervebudakk/Bina-Yonetim-Sistemi-Mobil)</sub> | `C#` `ASP.NET Core` `MS SQL` `Flutter` | Full-stack corporate architecture developed for Kayseri Şeker. Features a strictly typed RESTful API with secure JWT-Based Role Access Control (RBAC), a highly normalized relational database schema, and a cross-platform mobile interface. |
...
| Project Name | Primary Technologies | Description |
| :--- | :--- | :--- |
| **PULSE — Local News Monitoring Platform** <br> <sub>[Source Code](https://github.com/kou-dev-projects/KocaeliNewsMap)</sub> | `FastAPI` `Next.js` `MongoDB` `NLP` `Docker` | AI-powered civic monitoring platform that scrapes Kocaeli local news sources, categorizes events via an NLP pipeline (NER with BERTTurk / GLiNER, geocoding, duplicate detection), and visualizes them on an interactive GPU-accelerated map (MapLibre GL + Deck.gl) with real-time SSE streaming and PWA support. |
| **Word Crush — Turkish Word Puzzle Game** <br> <sub>*Private Repository*</sub> | `Flutter` `NestJS` `Supabase` `TypeScript` | Cross-platform Turkish word-finding game where players form words by connecting adjacent letters on a dynamic grid. Features a custom joker system with 6 power-up types, combo scoring, Google OAuth, global leaderboard, and a custom GLSL fragment shader for UI effects. Backend built with NestJS 11 and PostgreSQL via Supabase. |
